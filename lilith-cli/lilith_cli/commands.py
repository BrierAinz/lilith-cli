...
class MetricsCommand(BaseCommand):
    ...
    Delegates to ``run_metrics_command`` in extra_commands (which carries
    the canonical implementation: richer render, ``/metrics json``
    support, telemetry-off detection). Kept here so the CommandRegistry
    can discover the command via class registration.
    """
    ...
    async def execute(self, args: str) -> None:
        from .extra_commands import run_metrics_command

        await run_metrics_command(self.session, args)
