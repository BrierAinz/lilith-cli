...
async def run_diffconfig_command(session: AgentSession, args: str) -> None:  # noqa: ARG001
    ...
    current_mode = _load_diff_config().get("mode", "ask")

    if not text or text.lower() in ("show", "status"):
        console.print(f"[info]Modo de confirmación actual: [model]{current_mode}[/]")
        return

    if text.lower() in valid_modes:
        _save_diff_config({"mode": text.lower()})
        console.print(f"[success]✓ Modo de confirmación cambiado a: {text.lower()}[/]")
        return

    render_error(f"Modo inválido. Opciones: {', '.join(valid_modes)}")


def _load_diff_config() -> dict[str, Any]:
    """Carga configuración de diff desde ~/.yggdrasil/diff_config.json."""
    if not _DIFF_CONFIG_FILE.exists():
        return {}
    try:
        data = json.loads(_DIFF_CONFIG_FILE.read_text(encoding="utf-8"))
        if isinstance(data, dict):
            return data
    except Exception as exc:  # pragma: no cover
        logger = logging.getLogger(__name__)
        logger.warning("Error cargando diff config: %s", exc)
    return {}


def _save_diff_config(config: dict[str, Any]) -> None:
    """Guarda configuración de diff en ~/.yggdrasil/diff_config.json."""
    _DIFF_CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
    _DIFF_CONFIG_FILE.write_text(
