...
class TestEstimateContextWindow:
    ...
    def test_known_model(self) -> None:
        ...
        assert estimate_context_window("deepseek-v4-flash") == 1_000_000

    def test_unknown_model_falls_back(self) -> None:
        from lilith_cli.providers import estimate_context_window
        # Unknown models get the safe default of 128k.
        assert estimate_context_window("totally-unknown-model-xyz") == 128_000

    def test_none_model_falls_back(self) -> None:
        from lilith_cli.providers import estimate_context_window
        assert estimate_context_window(None) == 128_000

    def test_known_grok_model(self) -> None:
        from lilith_cli.providers import estimate_context_window
        assert estimate_context_window("grok-4.5") == 500_000
...
class TestRenderContext:
    ...
    def _make_session(self) -> MagicMock:
        from lilith_cli.session_telemetry import SessionTelemetry

        sess = MagicMock()
        sess.config = MagicMock()
        sess.config.model = "deepseek-v4-flash"
        sess.telemetry = SessionTelemetry()
        sess.telemetry.merge_usage(
            {
                "prompt_tokens": 1_000,
